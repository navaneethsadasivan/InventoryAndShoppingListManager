...
class AjaxController extends Controller
{
    ...
    {
        return response()->json(['items' => UserInventory::getUserInventory()], 200);
    }
    ...
    public function postAddItem(Request $request)
    {
        ...
        if ($request->getContent()) {
            $response = UserInventory::postAddItem($request->getContent());
        }
        return $response;
    }
    ...
    public function postRemoveItem(Request $request)
    {
        $response = null;
        if ($request->getContent()) {
            $response = UserInventory::postRemoveItem($request->getContent());
        }
        return $response;
    }
}
